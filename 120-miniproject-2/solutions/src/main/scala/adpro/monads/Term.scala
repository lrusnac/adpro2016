  ...
  def eval(term: Term): M[Int] = term match {
    ...
    case Div(t, u) => eval(t) match {
      ...
      case Return(a) => eval(u) match {
        case Raise(e) => Raise(e)
        case Return(b) => if (b == 0) Raise("divided by zero") else Return(a / b)
      }
    }
  }
...
//    } yield r
    ...
    case Div (t,u) => eval(t).flatMap(a => eval(u).flatMap(b => if(b == 0) Raise("divided by zero") else M.unit(a/b)))
  }
...
object StateEvaluatorWithMonads {
  ...
  def tick(): M[Unit] = M[Unit]((x:Int) => ((), x+1))

...
object Par {
  ...
  def sequence[A] (ps: List[Par[A]]): Par[List[A]] = ps.foldRight(lazyUnit(List[A]()))( (a,aa) => map2(a,aa)(_::_))
  ...
  // this is shown in the book:
   ...
   def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = fork {
     def Listify = (x: A) => if (f(x)) List(x) else List()
     val fbs: List[Par[A]] = as.map(asyncF(x => Listify(x))).flatten
     sequence(fbs)
   }
  ...
  //run(:ExecutorServce)(:Par[A])
  def choiceN[A] (n: Par[Int]) (choices: List[Par[A]]) :Par[A] = x => run(x)(choices(run(x)(n).get()))


  def choice[A] (cond: Par[Boolean]) (t: Par[A], f: Par[A]) : Par[A] = es => choiceN()(List(t,f))
